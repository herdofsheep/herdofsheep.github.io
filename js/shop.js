...
function update() {
  ...
  for (var i = 0; i < images.length; i++) {
    images[i].style.transform =
      "translateX(" + translateValue + "%) scale(" + 0.8 + ")";
  }

  images[focusPos].style.transform =
    "translateX(" + translateValue + "%) scale(" + 1 + ")";
...
function updateWidth() {
  ...
  for (var i = 0; i < images.length; i++) {
    images[i].style.maxWidth =
      "" + document.getElementById("imageFocus").clientWidth + "px";
  }
