...
class SimplerRayCast extends LitElement {
  ...
  init() {
    ...
    this.thicQue = this.loadModel(
      "/src/assets/models/gltf/questionmarkBig.glb",
    );
  ...
  duplicate() {
    ...
    if (found && foundThic) {
      ...
      var queClone = this.que.children[0].children.find(
        (x) => x.type == "Mesh",
      ).geometry;
      var thiccQueClone = this.thicQue.children[0].children.find(
        (x) => x.type == "Mesh",
      ).geometry;
