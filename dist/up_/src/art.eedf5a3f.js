function fairyDustCursor(options) {
    ...
    function onTouchMove(e) {
        if (e.touches.length > 0) for(let i = 0; i < e.touches.length; i++)addParticle(e.touches[i].clientX, e.touches[i].clientY, canvImages[Math.floor(Math.random() * canvImages.length)]);
    }
    ...
    function addParticle(x, y, color) {
        if (Math.floor(Math.random() * 10) == 0) particles.push(new Particle(x, y, color));
    }
...
//# sourceMappingURL=art.eedf5a3f.js.map
