...
function init() {
    ...
    for(var i = 0; i < links.length; i++)links[i].addEventListener("click", linkClick);
...
function linkClick(e) {
    ...
    if (link == "") link = e.target.parentElement.id;
    focusItems = document.getElementsByClassName("focus");
    if (focusItems.length > 0) for(var i = 0; i < focusItems.length; i++)focusItems[i].classList.remove("focus");
    e.target.classList.add("focus");
    document.getElementById("artMain").scrollLeft = 0;
    for(var i = 0; i < contentOptions.length; i++)contentOptions[i].style.visibility = "hidden";
    if (contentOptions[link] != undefined) contentOptions[link].style.visibility = "visible";
}

//# sourceMappingURL=art.2c0289ab.js.map
